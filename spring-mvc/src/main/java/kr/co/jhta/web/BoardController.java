 ...
 * 			- 클래스에 부착할 경우, 해당 클래스의 요청 핸들러 메소드의 매핑 URI의 접두사로 추가된다.
 ...
 * 						클라이언트 측에서 기대되는 응답 컨텐츠의 타입을 JSON이라고 지정한 경우
 * 						이 요청핸들러 메소드와 매핑된다.
 * 		@RequestParam
 * 			- 요청파라미터 값을 매개변수와 매핑시키는 어노테이션이다.
 * 			- 메소드의 매개변수 선언 앞에 부착한다.
 * 			- HandlerAdapter가 요청 핸들러 메소드의 @RequestParam과 매개변수를 분석해서 적절한 요청파라미터 값을 찾아서 매개변수에 전달한다.
 * 			- 주요 속성
 * 				value 
 * 					요청파라미터 명을 지정한다.
 * 					value="userid"
 ...
 * 					* 위 세가지 모두 요청파라미터명이 userId인 요청파라미터값을 찾아서 매개변수 id에 전달한다.
 ...
 * 					value의 사용법과 동일하다.
 * 				required
 * 					필수 요청파라미터 여부를 지정한다. 기본 값은 true다.
 * 					required=false로 지정하지 않은 이상 무조건 요청파라미터가 존재해야 한다.
 * 					required=false로 지정하는 경우 거의 대부분 defaultValue와 같이 사용된다.
 * 				defaultValue
 * 					요청파라미터 값이 비어 있을 때 기본 값을 지정한다.
 * 					required=false로 지정된 경우 defaultValue를 사용해서 기본 값을 지정할 수 있다.
...
public class BoardController {
	...
	public String ExceptionHandler(Exception e) {
		e.printStackTrace();
		return "error/server-known-error";
	}
	...
	// * <form /> 태그의 모든 입력필드값을 저장하는 폼 커맨드 객체를 요청핸들러 메소드의 매개변수로 선언하면,
	//	 HandlerAdapter가 요청파라미터값을 조회해서 폼 커맨드 객체에 자동으로 저장한 후 요청핸들러 메소드 실행 시 전달해준다.
	...
	public String addBoard(BoardForm boardForm, User user) throws FileNotFoundException, IOException {
		...
		// * 원본 객체의 필드값을 대상객체의 필드에 복사한다.
		// * 원본 객체의 필드명과 대상객체의 필드명이 동일한 필드만 복사가 발생한다.
		// * 원본객체와 대상객체의 필드명은 동일하지만, 타입이 서로 다를 때는 에러가 발생한다.
		// * 원본객체와 대상 객체의 필드를 선택적으로 복사할 수 없다.
		//   (필드명은 동일하지만, 타입이 다른필드가 존재하지 않아야 한다.)
		// title, writer, content, password 값이 boardForm에서 board로 복사도니다.
		...
		if(!upfile.isEmpty()) {
		// 3. 첨부파일이 존재하면, 업로드된 첨파일의 이름을 조회한다.
		...
		// 6. FileCopyUtils.copy(소스,대상)를 사용해서 서버의 임시디렉토리에 저장된 첨부파일을 File 객체가 지정한 위치로 복사한다.
			FileCopyUtils.copy(upfile.getInputStream(), new FileOutputStream(file)); // getInputStream으로 첨부파일 내용 읽어오기
		// 7. 파일명을 table에 저장하기 위해서 파일명을 VO 객체에 추가한다.
	...
	public ModelAndView downloadFile(@RequestParam("no") long boardNo, Model model) {
