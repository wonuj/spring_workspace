		...
		<div class="col-12">
			<div class="card">
				<form:form method="post" action="signin.do" modelAttribute="loginForm">
					...
					<div class="card-body">
					<c:choose>
						<c:when test="${param.error eq 'fail' }">	<!-- param. -> url의 파라미터값을 가져옴 -->
						...
						<c:when test="${param.error eq 'deny' }">
							<div class="alert alert-danger">
								<strong>서비스 거부</strong> 해당 서비스는 로그인 후 사용이 가능합니다.
							</div>
						</c:when>
					</c:choose>
						<div class="form-group">
							...
							<form:errors path="userId" cssClass="text-primary"></form:errors>
						</div>
						<div class="form-group">
							<label>비밀번호</label>
							<form:password path="userPwd" id="user-pwd" class="form-control" />
							<form:errors path="userPwd" cssClass="text-primary"></form:errors>
						</div>
					</div>
