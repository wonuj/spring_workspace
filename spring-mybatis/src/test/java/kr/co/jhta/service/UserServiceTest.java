 ...
 * 		- 테스트 대상 객체가 정의되어 있는 스프링 빈 설정파일의 경로를 지정하는 어노테이션
 * @RunWith
 * 		- JUnit 테스트가 실행될 때 같이 실행될 Helper 클래스를 지정하는 어노테이션
 ...
 * 		   테스트 작업 과정에서 수행한 INSERT/UPDATE/DELETE 작업을 Database에 반영시키지 않고
 * 		   각 개별 테스트 메소드의 실행이 종료되면 즉시 롤백시킨다.
 * 		- 테스트 수행 후 테스트 과정에서 database에 저장된 데이터를 직접 제거 하지 않아도 된다.
...
public class UserServiceTest {
	...
	public void testAddNewUser() {
		User user = new User("hong1234", "홍길동", "zxcv1234", "hong@gmail.com");
